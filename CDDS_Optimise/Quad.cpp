...
bool Quadtree::Insert(GameObject* gameObject) {
	if (m_boundary.contains(gameObject->m_bounds.m_centre) == false) return false;

	// if this node doesn't have children (hasn't been subdivided), then try to insert the gameobject in this node's object lise
	if (m_children == nullptr) {
		...
		if (m_objects[m_capacity - 1] == nullptr) {
			// we have at least 1 free space, so step through the array and look for somewhere to insert the data
		...
		// if we coudldn't insert it in the objects array (ie, it was full), then subdivide this node
		Subdivide();
	}

	// we couldn't insert the gameobject into this node, so find a child node to insert into
	for (int i = 0; i < 4; i++) {
		if (m_children[i]->Insert(gameObject) == true) return true;
	}
...
void Quadtree::Subdivide() {
	...
	if (m_objects != nullptr) {
		for (int i = 0; i < m_capacity; i++) {
			...
			for (int j = 0; j < 4; j++) {
				if (m_children[j]->Insert(m_objects[i]) == true) break;
			}

			// remove the gameobejct from the objects array for this node
			// (dont delete it, just set it to null - the object still needs to exist in the child node)
			m_objects[i] = nullptr;
		}
		// all of the objects that were stored in this node are now stored in the child nodes, so delete the objects array.
		delete m_objects;
		m_objects = nullptr;
	}
}

void Quadtree::Update(float delta) {
	// update local objects in this node
	if (m_objects != nullptr) {
		for (int i = 0; i < m_capacity; i++) {
			if (m_objects[i] != nullptr) {
...
void Quadtree::Draw()
{
	...
	if (m_objects != nullptr) {
		for (int i = 0; i < m_capacity; i++) {
			if (m_objects[i] != nullptr) m_objects[i]->Draw();
		}
	}
}

void Quadtree::Query(AABB area, std::vector<GameObject*>& foundObjects) {
	// if the search area doesn't overlap this quad, return
	if (!m_boundary.intersects(area)) return;

	// check objects at this node level
	if (m_objects != nullptr) {
		for (int i = 0; i < m_capacity; i++) {
			if (m_objects[i] != nullptr) {
				if (area.contains(m_objects[i]->m_bounds.m_centre)) {
	...
	if (m_children != nullptr) {
		for (int i = 0; i < 4; i++) {
			m_children[i]->Query(area, foundObjects);
		}
	}
}
